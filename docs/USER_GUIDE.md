...
| **Show grid lines** | off (default) · on | Draws dashed lines through the sub-point rows and columns so the points read as a connected grid. Display-only — toggling it never affects labels. |
